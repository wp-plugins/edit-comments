...
function jal_edit_comment($text = 'edit', $before = '', $after = '', $editing_message = '<strong>EDITING</strong>') {
	global $comment, $jal_minutes;
	$time_ago = time() - strtotime($comment->comment_date);
	...
	if ($comment->comment_author_IP == $_SERVER['REMOTE_ADDR']
	 ...
	 && !isset($_GET['jal_edit_comments']))
		echo "\n{$before}<a href='".add_query_arg('jal_edit_comments', "$comment->comment_ID")."#commentform'>{$text}</a>{$after}\n";
...
function jal_do_edit () {
	...
	if ($comment->comment_author_IP == $_SERVER['REMOTE_ADDR']
	 && $time_ago < 60 * $jal_minutes) {
		...
		die("You aren't allowed to edit this comment, either because you didn't write it or you passed the 30 minute time limit.");
}
